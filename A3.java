...
public class A3  extends JFrame {
	...
	class RemoveActionListener implements ActionListener{
		public void actionPerformed (ActionEvent e){
			Object selectPath = tree.getLastSelectedPathComponent();
			if(selectPath ==null){
				JOptionPane.showMessageDialog(null, "ERROR: No node selected.");
				System.out.println("ERROR: No node selected.");
			}else{
				if (selectPath != panel.getRoot()){
					panel.removeNodeFromParent ((Shape)selectPath);
				}else {
	...
	class AddActionListener implements ActionListener{
		public void actionPerformed (ActionEvent e) {
			...
			if(selectPath ==null){
				...
				if (selectPath instanceof NestedShape){
					panel.addShapeNode( (NestedShape)selectPath);
				}else if (!(selectPath instanceof NestedShape)){
